...
// Copyright (c) 2019, The CuddleCoin Developers
//
...
namespace CryptoNote
{
    namespace parameters
    {
        ...
        const uint64_t LWMA_2_DIFFICULTY_BLOCK_INDEX                 = 100000;  //v3
        const uint64_t LWMA_2_DIFFICULTY_BLOCK_INDEX_V2              = 200000;  //v4
        const uint64_t LWMA_2_DIFFICULTY_BLOCK_INDEX_V3              = 300000;  //v5

        const unsigned EMISSION_SPEED_FACTOR                         = 22;
        const uint64_t GENESIS_BLOCK_REWARD                          = UINT64_C(0);
        static_assert(EMISSION_SPEED_FACTOR <= 8 * sizeof(uint64_t), "Bad EMISSION_SPEED_FACTOR");

        const char     GENESIS_COINBASE_TX_HEX[]                     = "";
        static_assert(
        ...
        const uint32_t MIXIN_LIMITS_V1_HEIGHT                        = 1500;
        const uint32_t MIXIN_LIMITS_V2_HEIGHT                        = 100000;
        const uint32_t MIXIN_LIMITS_V3_HEIGHT                        = 300000;

        /* The mixin to use by default with zedwallet and cuddle-service */
        ...
        const uint64_t MAX_EXTRA_SIZE_V2                             = 1024;    //1k Extra
        ...
        const size_t   NORMAL_TX_MAX_OUTPUT_COUNT_V1_HEIGHT          = 0;
        const uint64_t MINIMUM_FEE_PER_BYTE_V1_HEIGHT                = 0;      // Height for our first fee to byte change to take effect.
        const uint64_t COINBASE_TRANSACTION_OUTPUT_CLAIMING_HEIGHT   = 2'800'000;      // Coinbase transactions must include the recipient address + tx priv

        /* 4,477,500 ARMS -> Max supply / mixin+1 outputs                 */
        /* This is enforced on the daemon side. An output > 4,477,500 causes an invalid block.   */
        const uint64_t MAX_OUTPUT_SIZE_NODE                          = 19'657'935'000'00;
        ...
        const uint32_t UPGRADE_HEIGHT_V5                             = 4;  // Upgrade height for CN_Armor Variant 2 switch (never used).
        const uint32_t UPGRADE_HEIGHT_V6                             = 5;  // Upgrade height for ARGON2 switch.
    ...
    const uint64_t ROCKSDB_READ_BUFFER_MB                        = 128;           // Recommended - 128 MB
    ...
    const char     LATEST_VERSION_URL[]                          = "https://github.com/cuddlecoinxo/cuddlecoin/releases/latest";
    const std::string LICENSE_URL                                = "https://github.com/cuddlecoinxo/cuddlecoin/blob/development/LICENSE";

    /* MainNet Network Identifier for CuddleCoin */
    const static   boost::uuids::uuid CRYPTONOTE_NETWORK         =
    {
        { 0x11, 0xff, 0x4c, 0xc3, 0xd2, 0xaa, 0x1d, 0x59, 0xab, 0x57, 0xef, 0x66, 0xaa, 0xce, 0x1f, 0x88 }
    };
