...
# Multi-installer
# a one line clone-and-compile for cuddlecoin:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/cuddlecoinxo/cuddlecoin/development/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/cuddlecoin ]; then
            read -r -p "${1:-cuddlecoin directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old cuddlecoin directory" && echo
                    rm -rf "$PWD"/cuddlecoin
                    ;;
                *)
                    _fail "cuddlecoin directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir cuddlecoin
        git clone -b development https://github.com/cuddlecoinxo/cuddlecoin cuddlecoin   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd cuddlecoin
    fi
}

_build_cuddlecoin() {
    _note "Building cuddlecoin from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/cuddlecoinxo')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/cuddlecoinxo')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "CuddleCoin Multi_Installer v1.0 (angus)"
...
_note "Look in 'cuddlecoin/build/src/' for the executible binaries. See 'https://github.com/cuddlecoinxo/cuddlecoin' for more project support. Good Job Man!"
